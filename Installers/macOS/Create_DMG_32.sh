...
cd $dmgDir
rm -f PhoediX_x86.dmg
rm -f pack.temp.dmg

hdiutil create -srcfolder "$forInstaller" -volname "PhoediX_x86" -fs HFS+ -format UDRW "pack.temp.dmg"
device=$(hdiutil attach -readwrite -noverify -noautoopen "pack.temp.dmg" | egrep '^/dev/' | sed 1q | awk '{print $1}')
sleep 3
echo '
   tell application "Finder"
     tell disk "'PhoediX_x86'"
           open
...
chmod -Rf go-w /Volumes/PhoediX_x86
sync
sync
hdiutil detach ${device}
hdiutil convert "pack.temp.dmg" -format UDZO -imagekey zlib-level=9 -o "PhoediX_x86.dmg"
